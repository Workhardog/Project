...
namespace PassthroughCameraSamples.MultiObjectDetection
{
    ...
    public class SentisInferenceRunManager : MonoBehaviour
    {
        ...
        internal static void PreloadModel(ModelAsset modelAsset)
        {
            ...
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel ModelLoader.Load ===");
            var inputShape = model.inputs[0].shape;
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel inputShape = model.inputs ===");
            // Create engine to run model
            using var worker = new Worker(model, BackendType.GPUCompute);
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel worker = new Worker(model ===");
            ...
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel tempTexture ===");
            var textureTransform = new TextureTransform().SetDimensions(tempTexture.width, tempTexture.height, 3);
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel textureTransform ===");
            using var input = new Tensor<float>(new TensorShape(1, 3, inputShape.Get(2), inputShape.Get(3)));
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel Tensor ===");
            TextureConverter.ToTensor(tempTexture, input, textureTransform);
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel ToTensor ===");
            if (input == null) { Debug.Log("Null input"); }
            if (worker == null) { Debug.Log("null worker"); }
            if (model == null) { Debug.Log("null model"); }
            if (modelAsset == null) 
            {
                 Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel null modelAsset==="); 
            }
            else
            {
                Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel have modelAsset==="); 
            }
            worker.Schedule(input);
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel worker.Schedule===");
            // Complete the inference immediately and destroy the temporary texture
            worker.PeekOutput(0).CompleteAllPendingOperations();
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel .PeekOutput(0)===");
            worker.PeekOutput(1).CompleteAllPendingOperations();
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel PeekOutput(1)===");
            worker.PeekOutput(2).CompleteAllPendingOperations();
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel PeekOutput(2)===");
            Destroy(tempTexture);
            Debug.Log("=== [DEBUG] SentisInferenceRunManager: PreloadModel Destroy and end===");
        }
